    ...
    <div className="eg-route-stack eg-route-progression">
      <SplitGrid>
        <Card
          ...
          title="War Room"
          subtitle="今すぐ処理する対象を上に固定する"
          actions={<Badge tone={claimableMissions.length > 0 || readyExpeditions.length > 0 ? "good" : "accent"}>Urgent</Badge>}
        >
          <div className="eg-urgent-grid">
            ...
            <div className="eg-urgent-column">
              ...
              {readyExpeditions.length > 0 ? (
                <PanelList>
                  {readyExpeditions.slice(0, 3).map((dispatch) => {
        ...
        <Card className="eg-growth-brief-card" title="Growth Reserves" subtitle="現在の伸ばし元">
          ...
          <PanelList>
            <PanelRow
              primary={`Tap Damage x${getUpgradeEffect("tapDamage", save.snapshot.meta.upgrades.tapDamage).toFixed(2)}`}
              secondary={`Next cost ${formatNumber(getUpgradeCost(save.snapshot.meta.upgrades.tapDamage))} gold`}
            />
            <PanelRow
              primary={`Hero DPS x${getUpgradeEffect("heroDps", save.snapshot.meta.upgrades.heroDps).toFixed(2)}`}
              secondary={`Next cost ${formatNumber(getUpgradeCost(save.snapshot.meta.upgrades.heroDps))} gold`}
            />
            <PanelRow
              primary={`Formation ${content.formations.find((entry) => entry.id === save.snapshot.meta.activeFormationId)?.name ?? "None"}`}
              secondary={`Completed Missions ${completedMissions.length}`}
            />
          </PanelList>
        </Card>
      </SplitGrid>
      ...
      <SplitGrid>
        <Card className="eg-growth-track-card" title="Growth Tracks" subtitle="押す前に必要資源と次の値を見せる">
          <div className="eg-subsection-heading">
            <span>Core Systems</span>
            <Chip tone="accent">Gold / Gems</Chip>
          </div>
          <PanelList>
            <PanelRow
        ...
        <Card className="eg-ascension-card" title="Ascension Lab" subtitle="転生系の恒久成長">
          <div className="eg-subsection-heading">
            <span>Talents</span>
            <Chip tone="accent">{formatNumber(save.snapshot.meta.prestigePoints)} pt</Chip>
          </div>
          <PanelList>
            {content.talents.slice(0, 5).map((talent) => {
          ...
          <PanelList>
            {content.artifacts.slice(0, 5).map((artifact) => {
